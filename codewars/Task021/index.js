...
// new string is "(" if that character appears only
...
// display in some languages.If you read "...It Should encode XXX",
// the "XXX" is the expected result, not the input!

function duplicateEncode(word) {
  const arrWordChek = [...word.toLowerCase()];
  const arrWord = [...word.toLowerCase()];
  const resArr = [];
  for (let i = 0; i < arrWord.length; i += 1) {
    arrWordChek[i - 1] = arrWord[i - 1];
    arrWordChek[i] = 'aa';
    if (arrWord[i] === ' ') {
      resArr.push('(');
    }
    if (arrWordChek.includes(arrWord[i])) {
      resArr.push(')');
    }
    if (!arrWordChek.includes(arrWord[i])) {
      resArr.push('(');
...
console.log(duplicateEncode('Dccd'));
